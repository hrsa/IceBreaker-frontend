...
export type ToSContent = string | string[];

export type ToSSubsection = {
  title: string;
  content: ToSContent;
  subsections?: Record<string, ToSSubsection>;
};
...
export default function ToSScreen() {
  ...
  const TOS_EFFECTIVE_DATE = new Date("2025-05-26");
  const tosData = tObj("tos:sections");
      ...
      <ScrollView contentContainerStyle={styles.scrollView}>
        <View style={styles.content}>
          <View style={styles.header}>
            <Text style={styles.title}>{t("tos:title")}</Text>
            <Text style={styles.subtitle}>
              {t("tos:effectiveDate", {
                date: TOS_EFFECTIVE_DATE.toLocaleDateString(language, {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                }),
          ...
          {Object.entries(tosData).map(([key, section], index) => renderSection(key, section, index))}
